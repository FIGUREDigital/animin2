...
public class RecordingGUI : MonoBehaviour
{
    ...
    void OnGUI()
    {
             ...
             Application.platform == RuntimePlatform.Android))
        {
            if ( GUI.Button(showWatchViewButtonRect, "Show Watch View", style) )
            {
				...
				/*
				IList<Kamcord.KCMetadata> constraints = new List<Kamcord.KCMetadata>();
                Kamcord.KCMetadata metadata1 = new Kamcord.KCMetadata("X面");
                Kamcord.KCMetadata metadata2 = new Kamcord.KCMetadata("ボス戦");

                constraints.Add(metadata1);
                constraints.Add(metadata2);

                Kamcord.ShowFeedWithMetadataConstraintsAndTitle(constraints, "The Title");
                */
            }
        }
        ...
        if (Kamcord.IsRecording() || Kamcord.IsPaused())
        {
            ...
            if (GUI.Button(stopRecordingButtonRect, "Stop Recording", style))
            {
                ...
                Kamcord.KCMetadata metadata1 = new Kamcord.KCMetadata("key1", "value1");
                Kamcord.KCMetadata metadata2 = new Kamcord.KCMetadata("key2", 42);
                Kamcord.KCMetadata metadata3 = new Kamcord.KCMetadata("key3", 31.0f);
                Kamcord.KCMetadata metadata4 = new Kamcord.KCMetadata("sometag");

                Kamcord.AddMetadataToVideo(metadata1);
                Kamcord.AddMetadataToVideo(metadata2);
                Kamcord.AddMetadataToVideo(metadata3);
                Kamcord.AddMetadataToVideo(metadata4);
    ...
    void MyVideoSharedToNicoNico(string videoID, bool success)
    {
