        ...
        */
        ...
        void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                if (isStrikethrough)
                {
                    if (beginStrikethrough == false && currentCharacter != 32 && currentCharacter != 10 && currentCharacter != 13)
                    {
                        beginStrikethrough = true;
                        strikethrough_start = new Vector3(m_textInfo.characterInfo[i].bottomLeft.x, m_textInfo.characterInfo[i].baseLine + (font.fontInfo.Ascender + font.fontInfo.Descender) / 2 * m_fontScale, 0);
                        underlineColor = m_textInfo.characterInfo[i].color;
                        //Debug.Log("Underline Start Char [" + m_textInfo.characterInfo[i].character + "].");
                    }
                    ...
                    if (m_characterCount == 1)
                    {
                        ...
                        strikethrough_end = new Vector3(m_textInfo.characterInfo[i].topRight.x, m_textInfo.characterInfo[i].baseLine + (font.fontInfo.Ascender + font.fontInfo.Descender) / 2 * m_fontScale, 0);

                        DrawUnderlineMesh(strikethrough_start, strikethrough_end, ref last_vert_index, underlineColor);
                        underlineSegmentCount += 1;
                    }
                    else if (i == lineInfo.lastCharacterIndex)
                    {
                        ...
                        {
                            int lastVisibleCharacterIndex = lineInfo.lastVisibleCharacterIndex;
                            strikethrough_end = new Vector3(m_textInfo.characterInfo[lastVisibleCharacterIndex].topRight.x, m_textInfo.characterInfo[lastVisibleCharacterIndex].baseLine + (font.fontInfo.Ascender + font.fontInfo.Descender) / 2 * m_fontScale, 0);
                        }
                        else
                        {
                            strikethrough_end = new Vector3(m_textInfo.characterInfo[i].topRight.x, m_textInfo.characterInfo[i].baseLine + (font.fontInfo.Ascender + font.fontInfo.Descender) / 2 * m_fontScale, 0);
